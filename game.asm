...
computeNextGeneration:
    ...
    ;This is done so that ds begins with vedio buffer(since compact it doest disturb CS)
    mov AX, offset videoBuffer; Load base address of video buffer into AX
    mov DX, DS;
    add AX, DX; Using compact model so we need to address using both segment and offset
...
push BX;
    ...
    ; First pixel at (x0, y0)
    mov xPixelArray[BX], CX;
    mov yPixelArray[BX], DX;
    
    ;Second pixel at (x0+1, y0)
    inc BX
    inc BX
    inc CX
    mov xPixelArray[BX],CX
...
setupLoop:
    ...
    ; call putBlockpattern
    ; call putBlinkerPattern
    ; call putBeaconpattern
